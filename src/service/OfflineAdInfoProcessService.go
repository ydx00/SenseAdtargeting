...
func OfflineAdStaticInfoProcess(){
	...
	log.Println("adList.size:"+strconv.Itoa(len(appIdList)))
		...
		for _,adv := range cptAdList{
			...
			if len(aAudienceOss) > 0 {
				for _,os := range aAudienceOss{
					adCons := util.AD_DM_SENSEAR_AD_STATIC_CPT + appId + ":" +adMode + ":" + strconv.Itoa(os)
						...
						for _,os := range aAudienceOss{
							for _,area := range aAudienceAreas{
								for _,bTag := range aBroadcasterTags{
									adCons := util.AD_DM_SENSEAR_AD_STATIC_CPM+appId+":"+adMode+":"+strconv.Itoa(gender)+"_"+strconv.Itoa(ageGroup)+"_"+strconv.Itoa(os)+"_"+strconv.Itoa(area)+"_"+bTag
...
func OfflineAdRealtimeInfoProcess(){
	...
	for _,appId := range appIdList{
		...
		for _,adv := range cptAdList{
			...
			checkMap["statusCheck"] = strconv.Itoa(status)
